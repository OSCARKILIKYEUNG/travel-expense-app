...
部分店家（如唐吉訶德）細項已是**免稅後單價**，加總與實付一致，無法用「標價−實付」推出退稅，但收據仍印「免稅額」。請在編輯填 **收據免稅額（僅顯示）**，或由 AI 填入 `receipt_tax_exemption_amount`，卡片會顯示以免誤解；**不影響**實付與分帳公式。
...
- **[docs/PRODUCT_MANAGEMENT.md](./docs/PRODUCT_MANAGEMENT.md)** — 產品路線圖、版本節奏、架構輕量優化優先級、技術債登記（**建議與程式一併維護**）；**[與 AI 協作 SOP](./docs/PRODUCT_MANAGEMENT.md#與-ai-協作-sop)**（意見→對齊→再實作）亦在本檔
